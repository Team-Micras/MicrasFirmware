/**
 * @file test_go_to_point.cpp
 *
 * @brief Test for the GoToPoint class
 *
 * @date 10/2024
 */
    ...
    TestCore::loop([&imu, &odometry, &go_to_point, &finished, &started, &locomotion, &argb, &button, &timer,
        ...
        if (not finished) {
            const auto& [linear, angular] =
                go_to_point.action(odometry.get_state(), goal, core::FollowWallType::RIGHT, elapsed_time);
