...
Battery::Battery(const Config& config) :
    adc{config.adc}, voltage_divider{config.voltage_divider}, filter{config.filter_cutoff} {
    this->adc.start_dma({&(this->raw_reading), 1});
}

void Battery::update() {
    this->filter.update(this->raw_reading);
}

float Battery::get_voltage() const {
    return this->voltage_divider * this->filter.get_last() * this->adc.reference_voltage / this->adc.get_max_reading();
}
