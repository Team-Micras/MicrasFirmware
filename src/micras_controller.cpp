...
static volatile float test_next_goal_x{};
static volatile float test_next_goal_y{};

static volatile float test_absolute_pose_x{};
static volatile float test_absolute_pose_y{};

static volatile float test_pose_x{};
static volatile float test_pose_y{};
static volatile float test_pose_orientation{};

static volatile float test_loop_time{};
static volatile float test_max_loop_time{};
...
void MicrasController::update() {
    ...
    switch (this->state) {
        ...
        case State::IDLE: {
            ...
            if (button_status == proxy::Button::Status::EXTRA_LONG_PRESS) {
                this->next_state = State::CALIBRATE;
            }
    ...
    test_loop_time = elapsed_time;

    if (test_loop_time > test_max_loop_time) {
        test_max_loop_time = test_loop_time;
    }
...
bool MicrasController::run(float elapsed_time) {
    ...
    test_next_goal_x = this->current_action.point.x;
    test_next_goal_y = this->current_action.point.y;

    test_absolute_pose_x = state.pose.position.x;
    test_absolute_pose_y = state.pose.position.y;

    test_pose_x = relative_state.pose.position.x;
    test_pose_y = relative_state.pose.position.y;
    test_pose_orientation = relative_state.pose.orientation;
