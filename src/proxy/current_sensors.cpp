...
CurrentSensors<num_of_sensors>::CurrentSensors(const Config& current_sensors_config) :
...
template <uint8_t num_of_sensors>
float CurrentSensors<num_of_sensors>::get_current(uint8_t sensor_index) {
    return reference_voltage * this->adc_buffer.at(sensor_index) / (max_adc_reading * this->shunt_resistor);
}

template <uint8_t num_of_sensors>
uint32_t CurrentSensors<num_of_sensors>::get_current_raw(uint8_t sensor_index) {
